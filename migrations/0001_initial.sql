...
CREATE TABLE assets (
  ...
  kind TEXT NOT NULL CHECK (kind IN ('wallpaper', 'site_icon')),
  variant TEXT CHECK (
    (kind = 'wallpaper' AND variant IN ('desktop', 'mobile'))
    OR (kind = 'site_icon' AND variant IS NULL)
  ),
...
CREATE UNIQUE INDEX idx_assets_site_icon_checksum
ON assets(checksum) WHERE kind = 'site_icon';
