...
import type { BackgroundEffect, BackgroundEffectMode, MainState } from "@/typings/store";
...
export default function Set() {
  ...
  const updateEffect = (effect: BackgroundEffect, checked: boolean) => set("selectedEffects", checked ? [...state.selectedEffects, effect] : state.selectedEffects.filter((item) => item !== effect));
  ...
  return <div className="setting"><div className="native-collapse"><details open><summary>个性壁纸</summary><div className="collapse-content"><RadioSet name="wallpaper" value={state.coverType} values={[[0, "默认壁纸"], [1, "每日一图"], [2, "随机风景"], [3, "随机动漫"]]} onChange={(value) => { set("coverType", value); toast.success("壁纸更换成功"); }} /><div className="item"><span className="text">自动切换</span><RadioSet name="auto-wallpaper" value={state.autoBGSwitchInterval} values={[[0, "关闭"], [1, "15 秒"], [2, "30 秒"], [3, "45 秒"]]} onChange={(value) => set("autoBGSwitchInterval", value)} /></div>{state.coverType === 0 && <div className="item"><span className="text">默认壁纸 ID</span><div className="inline-control"><input value={wallpaperId} onChange={(event) => setWallpaperId(event.target.value)} type="number" min="1" max={state.wallpaperMaxId || undefined} placeholder="随机" /><button type="button" onClick={applyWallpaper}>应用</button><button type="button" onClick={clearWallpaper}>随机</button></div></div>}</div></details><details><summary>个性化调整</summary><div className="collapse-content"><div className="item"><span className="text">建站日期显示</span><Toggle label="建站日期显示" checked={state.siteStartShow} onChange={(value) => set("siteStartShow", value)} /></div><div className="item effect-settings"><span className="text">背景特效</span><RadioSet name="effects-mode" value={state.effectsMode} values={[["auto", "自动"], ["off", "关闭"], ["manual", "手动"]] as Array<[BackgroundEffectMode, string]>} onChange={(value) => set("effectsMode", value)} /></div>{state.effectsMode === "manual" && <div className="item effect-settings"><span className="text">手动选择</span><div className="checkbox-group">{([['snow', '雪'], ['firefly', '萤火虫'], ['lantern', '灯笼'], ['meteor', '流星']] as Array<[BackgroundEffect, string]>).map(([effect, label]) => <label key={effect}><input type="checkbox" checked={state.selectedEffects.includes(effect)} onChange={(event) => updateEffect(effect, event.target.checked)} />{label}</label>)}</div></div>}<div className="item"><span className="text">底栏背景模糊</span><Toggle label="底栏背景模糊" checked={state.footerBlur} onChange={(value) => set("footerBlur", value)} /></div></div></details><details><summary>其他设置</summary><div className="collapse-content"><div className="text">暂时没有其它啦 qwq</div></div></details>{state.setV && <details><summary>开发设置</summary><div className="collapse-content"><DevSet /></div></details>}</div></div>;
}
