...
import { getPlayerList, resolveNeteasePlaybackUrl } from "@/api";
...
type PlaybackState = { trackId: string; url: string; source: "chksz" | "original" };
...
export default function Music() {
  ...
  const playbackUrl = current && playback?.trackId === current.id ? playback.url : undefined;
  ...
  useEffect(() => {
    ...
    setPlaylist([]); setIndex(0); setLyrics([]); setPlayback(null);
  ...
  useEffect(() => {
    ...
    if (musicConfig.server !== "netease" || !/^\d{1,20}$/.test(current.id)) {
      setPlayback({ trackId: current.id, url: current.url, source: "original" });
      return () => controller.abort();
    }

    void resolveNeteasePlaybackUrl(current.id, controller.signal)
      .then((url) => {
        if (!controller.signal.aborted) setPlayback({ trackId: current.id, url, source: "chksz" });
      })
      .catch(() => {
        if (!controller.signal.aborted) setPlayback({ trackId: current.id, url: current.url, source: "original" });
      });
    return () => controller.abort();
  }, [applyMetadata, current, musicConfig.server, setLyric]);

  useEffect(() => {
    if (playbackUrl) audio.current?.load();
  }, [playbackUrl]);
  ...
  const handlePlaybackError = () => {
    if (current && playback?.trackId === current.id && playback.source === "chksz") {
      const state = useMainStore.getState();
      shouldPlay.current = shouldPlay.current || state.playerStatus === "playing";
