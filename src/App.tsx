...
export default function App() {
  ...
  return <><Loading /><Background onLoadComplete={loadComplete} />{loaded && <main id="main"><div className="page-container"><section className={`all${ownerPanel ? " owner-panel-open" : ""}`}><MainLeft onOpenOwnerPanel={openOwner} />{!boxOpen && <MainRight ownerPanelOpen={ownerPanel} onCloseOwnerPanel={closeOwner} />}{boxOpen && <Box />}</section></div>{!ownerPanel && <><ThemeSwitcher /><button type="button" className="menu" aria-label={mobileOpen ? "关闭移动菜单" : "打开移动菜单"} onClick={() => patch({ mobileOpenState: !mobileOpen })}>{mobileOpen ? <CloseSmall theme="outline" size="24" /> : <HamburgerButton theme="outline" size="24" />}</button><Footer className="f-ter" /></>}</main>}</>;
}
